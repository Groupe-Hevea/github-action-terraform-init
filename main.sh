...
git diff -C "$ROOT_DIR" "$ROOT_DIR/$WORKING_DIR/.terraform.lock.hcl"
if [ "$exist_lock_file" = "false" ] || ! git diff --quiet -C "$ROOT_DIR" "$ROOT_DIR/$WORKING_DIR/.terraform.lock.hcl"; then
